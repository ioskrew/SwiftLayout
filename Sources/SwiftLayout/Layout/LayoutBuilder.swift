//
//  LayoutBuilder.swift
//  
//
//  Created by oozoofrog on 2022/02/06.
//

import Foundation

@resultBuilder
public struct LayoutBuilder {
    public static func buildBlock(_ components: Layout...) -> [Layout] {
        components
    }
    public static func buildArray(_ components: [[Layout]]) -> [Layout] {
        components.flatMap({ $0 })
    }
    public static func buildEither(first component: [Layout]) -> [Layout] {
        component
    }
    public static func buildEither(second component: [Layout]) -> [Layout] {
        component
    }
    public static func buildOptional(_ component: [Layout]?) -> [Layout] {
        component ?? []
    }
    
    public static func buildFinalResult(_ component: [Layout]) -> [Layout] {
        component.reduce(into: [Layout]()) { partialResult, layout in
            if let array = layout as? Array<Layout> {
                partialResult.append(contentsOf: array)
            } else {
                partialResult.append(layout)
            }
        }
    }
}
